...
The primary motivation for this paper was to investigate to what degree user experience can be detected using physiological sensors.\todo{Anders start her i morgen :)}
...
Furthermore we partly \textbf{H2} we showed that using the fusion method stacking we could achieve significantly better accuracy than than using a single sensor as prediction tool, with the exception of the Kinect on the groupings Arousal 3, Valence 2 High, and Valence 3.\todo{Go over this numbers again}
...
However since we have shown that we are able to some degree\todo{av, ved ikke om der en bedre formulering} predict the affective state of a person in low resolution, a natural step would be to use the technique in a context which did not require a high resolution. 
