...
\todo{I think the intro is donnodk now, besides explaining SAM, and an argument for why we use multiple sensors}


%- tool for evaluators
%   - aim to find points of interest
%       - can we find/predict points of interest within events where usability problems are seeded?
%       - how does the relationship between TP/FP change if we change parameters on the one-class SVM
%   - mention how evaluators could use this in a tool (to slide video-recordings to points of interest)
%
%- mention some methods, we choose one-class SVM (because such and such, den fitter purpose for vores use case):
%   - vi kan ikke bruge en hel almindelig svm (rigtig mere normal-data mod anomali-data, derfor er one-class bedst)
