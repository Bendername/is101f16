...
However, as Bruun et al.~\cite{LH-paper} mentions, the approaches used to
...
aloud}~\cite{use_of_TA_and_IDA} can
help observers better gauge a user's thoughts, and to some extend affective
state, while experiencing UPs, but does not formalise
capturing affective state in particular and is inherently a subjective
evaluation. Three predominant methods formalised
for capturing a user's affective state are: affective self-report, physiological
reactivity and observable behaviour~\cite{BRADLEY199449}. Strategies such as \textit{think
...
and is not yet regarded as common practice \todo{cite our own review here?}.
...
Bruun et al. [cite] formalized a method and formula for associating
