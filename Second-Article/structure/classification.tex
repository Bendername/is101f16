...
\begin{figure}[H]
  ...
  \includegraphics[width=0.75\columnwidth]{graphics/oneclass-svm.png}
    \caption{The circle ``Normal State'' is the area of which any data considered has to be inside, if it's ``normal''. The outlier will be labeled as an anomaly i.e. usability error for the user because it does not fit within the model/area of normalities. }
...
To decide whether an anomaly correctly corresponds to a usability error e.g. that it \textit{hits} a usability error, some assumptions have to be made. 
...
The scoring function is the harmonic mean of EHR and FCR which is a kind of averaging of the two rates.\todo{Det er lidt
``frækt'' at sige at ``det er sådan ca. det samme som det her'' - det bliver læseren ikke klogere af}
