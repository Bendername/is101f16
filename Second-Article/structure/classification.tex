...
This can be seen on Figure \ref{[FIGURE] OneClass SVM}\todo{Make a nicer one}.
...
Since the one-class SVM is sensitive\cite{oneClassSVM} to its parameter settings, a grid search is performed on them. The $Gamma = \{2^{-14},..,2^2\}$ value is, together with the Sigmoid and RBF kernel values. This is done to find the optimal settings where ``Nu parameter'' is kept at 0.05.\todo{Skriv at vi lavet et parameter search og smid det her i sammenfatningen}
...
To decide whether an anomaly correctly corresponds to a usability error e.g. that it \textit{hit}s a usability error, some assumptions have to be made. 
...
Since the Nu parameter indirectly dictates how much of the training data that should be inside the boundry of the ``normal state'', an examination of the Nu parameter impact on the results is conducted.\todo{Kan ikke lide den her sætning men har skrevet den anywho, lav evt et billede omkring det i stedet}
