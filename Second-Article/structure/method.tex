...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}[
    node/.style={rectangle, draw=black!60, fill=black!5, very thick, align=center, minimum height=20pt},
    edge/.style={-latex,line width=3pt, shorten >=2pt, shorten <=2pt, rounded corners}]
    \node[node] (start) {Rest period};
    \node[node] (unseeded) [right = 40pt of start] {Two unseeded tasks};
    \node[node] (tasks)    [below = 20pt of unseeded] {random seeded/unseeded tasks};
    \draw[edge] (start) -- (unseeded) node [midway, above = 2pt] (TextNode) {\small{3min}};
    \draw[edge] (unseeded) -- (tasks);
  \end{tikzpicture}
  \caption{test}
  \label{fig:test}
\end{figure}
