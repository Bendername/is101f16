...
affective state. How such affective states manifests themselves in physiological measurements, we deem to be an advanced
...
In this study, we work with the assumption that the relation \textit{one-many} holds for all individuals, from the
perspective that a particular physiological event can be manifested by many different psychological events, e.g. being
frustrated and happy could result in a similar increase in heart-rate. This further validates our argument for using
multiple sensors, in that the only way to separately identify feeling frustrated and happy might be dissimilar patterns
in electrodermal activity. If the \textit{one-many} relation holds, it is perhaps more likely that we are able to
capture and identify a particular psychological affective state if we use multiple and different types of physiological
sensors.
...
induced at very specific moments, leading us to approach the above differently.  \todo{cite our previous work somehow}
...
after Z seconds, and lasts for K seconds. \todo{change to correct values and add citations}
...
\todo{figure out wtf is section X, what we should call it and where to place it (method, approach?)} 
\todo{Vis nogle grafer for hvordan mange peaks ser ud efter et event, snak om latency og duration etc}
\todo{if we need graphs here, then what should they show?}
\todo{if we need graphs here, then what should they show?}
